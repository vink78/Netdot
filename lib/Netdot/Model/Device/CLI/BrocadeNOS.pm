...
sub get_arp {
	...
	$logger->debug(sub{"Device::BrocadeNOS::_get_arp: $host excluded from ARP collection. Skipping"});
	return;
    }
    if ( $self->is_in_downtime ){
	$logger->debug(sub{"Device::BrocadeNOS::_get_arp: $host in downtime. Skipping"});
	return;
    }
...
sub _get_arp_from_cli {
	...
	chomp($line);
	if ( $line =~ /^($IPV4)\s+($CISCO_MAC)\s+Ve\s+(\d+)\s+yes\s+(\S+)/ ) {
	    $ip    = $1;
	    $mac   = $2;
	    $iname = 'Vlan'.$3;
	}else{
...
sub _get_fwt_from_cli {
	...
	if ( $line =~ /^(\d+)\s+($CISCO_MAC)\s+Dynamic\s+\S+\s+Po\s+(\d+)/ ) { # VDX Syntax
	    ...
	    $logger->debug(sub{"Device::CLI::BrocadeNOS::_get_fwt_from_cli: line did not match criteria: '$line'" });
	    next;
	}
	...
	unless ( $intid ) {
	    $logger->warn("Device::CLI::BrocadeNOS::_get_fwt_from_cli: $host: Could not match $iname to any interface names");
	    next;
	}
	...
	if ( $validmac ){
	    ...
	    $logger->debug(sub{"Device::CLI::BrocadeNOS::_get_fwt_from_cli: $host: Invalid MAC: $mac" });
	    next;
	}	

	# Store in hash
	$fwt{$intid}{$mac} = 1;
	$logger->debug(sub{"Device::CLI::BrocadeNOS::_get_fwt_from_cli: $host: $iname -> $mac" });
    }
